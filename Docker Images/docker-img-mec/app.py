...
#MQTT Topics
@app.route('/mec_service_mgmt/v1/devices/<appId>/', methods=["GET",'PUT', 'POST','DELETE'])
def mqtt_devices(appId):
    global appids
    if request.method == 'POST':
        data = json.loads(request.data)
        for id in appids.keys():
            for s in appids[id]['servicelist']:
                if s['transportInfo']['type'] == 'MB_TOPIC_BASED' and s['transportInfo']['protocol'] == 'MQTT':
                    s['transportInfo']['endpoint']['alternative']["mqtt-topics"]['topics'].extend(data['topics'])
                    return "ok"

        abort(404, description="No valid MQTT endpoint found")
    else:
        return "TODO method"
