...
var _updateSensors = function (data) {
	console.log(data.sensors)
	return data.sensors
}

var _get_query_sensor = function (endpoint) {
	$.get(endpoint).done(_updateSensors);
}
...
var fetchSensorsInterval = null
var fetchSensorsDataInterval = null
/* UI related */
var activate_menu = function (menu) {
	...
	activeMenu = menu
	if (activeMenu === 'iotTab') {
		fetchSensorsInterval = setInterval(() => createSensorButtons(), 1000)
		fetchSensorsDataInterval = setInterval (() => getBackendData(), 1000)

	} else {
		clearInterval(fetchSensorsInterval)
		clearInterval(fetchSensorsDataInterval)
	}
}

var fetchSensors = function () {
	/*let try = _get_query_sensor("_get_sensors")
	console.log(try)*/
	/*return _get_query_sensor("_get_sensors")*/
	return ['dev423/temperature', 'dev423/air_quality']
}

var getBackendData = function (){
	_get_query("_acquire_data")
}

var createSensorButtons = function (){
	sensorsData = fetchSensors()
	let sensorButtons = $('#sensorButtons')
	sensorButtons.empty()
	console.log(sensorsData)
	sensorsData.forEach(sensorData => {
		let sensorButton = $(`<button>${sensorData}</button>`).click(function () {
			$.get("get_sensor_data/".concat(sensorData))
			console.log(sensorData)
		});
		sensorButton.addClass('btn btn-primary btn-lg sensor-btn')
		sensorButtons.append(sensorButton)
	})
}
