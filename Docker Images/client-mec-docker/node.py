...
class Node:
    ...
    def print_file(self, *args, **kwargs):
        with open("/usr/local/bin/result","a") as f:
            kwargs["file"] = f
            print(*args, **kwargs)
    ...
    def get_node_value(self):
        ...
        try:
            ...
            self.node_value=None
        self.print_file(self.node_value)
...
class ServerNodes:
    ...
    def show_hierarchie(self):
        ...
        for i in self.node_list:
            self.print_file(str(element) + "\t\t" + "|"*(len(i.hierarchie)-2), end="")
            if len(i.hierarchie) != 1:
                self.print_file("+"+"--"*(len(i.hierarchie)-1), end="")
            self.print_file(i.node_name[i.node_name.find(":")+1:])
            element +=1
    def show_values():
        for i in self.node_list:
            tmp=i.get_node_value()
            if tmp != None:
                self.print_file(str(i.hierarchie)+"\t"+i.node_name+":\t"+str(tmp))
